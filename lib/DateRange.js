...
var _mouseLeaveCallbacks = [];
var mouseLeaveNotice = function(cb){
  _mouseLeaveCallbacks.push(cb);
};

var _mouseOverDateCallbacks = [];
var mouseOverDate = function(cb){
  _mouseOverDateCallbacks.push(cb);
};
...
var DateRange = (function (_Component) {
  ...
  function DateRange(props, context) {
    ...
    this.state = {
      ...
      link: linkedCalendars ? (endDate ? endDate.clone() : (0, _moment2['default'])().startOf('day')) : false,
      step: props.step
    };
  ...
  _createClass(DateRange, [{
    ...
    value: function handleSelect(date, source) {
      ...
      if(range.startDate && range.startDate.isAfter(date)){
        step = 0;
      }
      switch (step){
        case 0:
          range.startDate = date;
          if(range.endDate && range.endDate.isBefore(date)){
            range.endDate = null;
          }
          step = 1;
          break;
    ...
    key: 'handleMouseOverDate',
    value: function handleMouseOverDate(date) {
      _mouseOverDateCallbacks.map(function(cb){
        cb(date);
      });
    }
  }, {
    ...
    value: function handleMouseLeave() {
      _mouseLeaveCallbacks.map(function(cb){
        cb();
      });
    }
  }, {
    ...
    value: function render() {
      ...
      return _react2['default'].createElement(
          'div',
          {
            style: onlyClasses ? undefined : _extends({}, styles['DateRange'], style),
            className: classes.dateRange,
            onMouseLeave: _this.handleMouseLeave.bind(_this)
          },
          ...
          (function () {
            var _calendars = [];
            for (var i = 0; i < Number(calendars); i++) {
              _calendars.push(_react2['default'].createElement(_CalendarJs2['default'], {
                ...
                mouseOverDate: _this.handleMouseOverDate.bind(_this),
                mouserOverDateListen: mouseOverDate,
                mouseLeave: mouseLeaveNotice
              }));
            }
            return _calendars;
          })()
