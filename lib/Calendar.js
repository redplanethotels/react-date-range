...
var Calendar = (function (_Component) {
  ...
  function Calendar(props, context) {
    ...
    var state = {
      ...
      shownDate: range && range['endDate'] || date ?
          (range && range['endDate'] || date).clone().add(offset, 'months') :
          (0, _moment2['default'])().startOf('day').add(offset, 'months'),
      firstDayOfWeek: firstDayOfWeek || _moment2['default'].localeData().firstDayOfWeek(),
      mouserOverOnDate: null
    };

    var _this = this;
    props.mouseLeave(function(){
      _this.setState({mouserOverOnDate: null});
    });

    props.mouserOverDateListen(function(date){
      _this.setState({mouserOverOnDate: date});
    });
  ...
  _createClass(Calendar, [{
    ...
    value: function getShownDate() {
      ...
      var shownDate = link && link.clone ? link.clone().add(offset, 'months') : this.state.shownDate;
    ...
    key: 'handleMouseOver',
    value: function handleMouseOver(newDate) {
      this.props.mouseOverDate(newDate);
    }
  }, {
    key: 'handleMouseLeave',
    value: function handleMouseLeave() {
      this.setState({mouserOverOnDate: null});
    }
  }, {
    key: 'changeMonth',
    ...
    value: function renderMonthAndYear(classes) {
      ...
      var monthAndYear = _react2['default'].createElement(
          'span',
          null,
      ...
      var emptyPrevButton = _react2['default'].createElement(
          'div',
          {
            style: {
              float: 'left',
      ...
      var emptyNextButton = _react2['default'].createElement(
          'div',
          {
            style: {
              float: 'right',
      ...
      return _react2['default'].createElement(
          ...
          prevButton, monthAndYear, nextButton
      );
    }
  }, {
    key: 'renderWeekdays',
    ...
    value: function renderDays(classes) {
      ...
      var mouserOverOnDate = _state.mouserOverOnDate;

      var dateUnix = date ? date.unix() : null;
      ...
      return days.map(function (data, index) {
        ...
        if(range && range.startDate && !range.endDate && mouserOverOnDate && !isInRange){
          isInRange = dayMoment.isBetween(range.startDate, mouserOverOnDate)
              || dayMoment.isBetween(mouserOverOnDate, range.startDate);
        }

        return _react2['default'].createElement(_DayCellJs2['default'], _extends({
          onSelect: _this.handleSelect.bind(_this),
          onMouserOver: _this.handleMouseOver.bind(_this)
        }, data, {
          theme: styles,
...
Calendar.propTypes = {
  ...
  mouseLeave: _react.PropTypes.func,
  mouseOverDate: _react.PropTypes.func
};
